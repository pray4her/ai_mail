...
CALL add_column_if_missing('mail_message', 'is_read', '`is_read` TINYINT(1) NOT NULL DEFAULT 0');
CALL add_column_if_missing('mail_message', 'is_flagged', '`is_flagged` TINYINT(1) NOT NULL DEFAULT 0');
...
CREATE TABLE IF NOT EXISTS mail_processing_record (
  id BIGINT NOT NULL AUTO_INCREMENT,
...
CALL add_column_if_missing('kb_vector_index', 'embedding_vector', '`embedding_vector` LONGTEXT NULL COMMENT ''向量数据(JSON 格式，仅测试使用)'' AFTER `embedding_id`');
